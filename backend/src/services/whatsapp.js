...
async function clearSession(profileId) {
  try {
    ...
    await db.query('DELETE FROM whatsapp_sessions WHERE profile_id=$1', [profileId]);
  } catch (err) {
    console.error(`clearSession error for ${profileId}:`, err.message);
  }
}
...
async function getAIReply(profile, incomingMessage, customerPhone) {
  try {
    ...
    const modelMap = {
      'gemini-flash-lite': 'gemini-1.5-flash-8b',
      'gemini-flash': 'gemini-1.5-flash',
      'gemini-pro': 'gemini-1.5-pro'
    };
    const modelName = modelMap[profile.gemini_model] || 'gemini-1.5-flash';
    console.log(`Using Gemini model: ${modelName} for profile ${profile.profile_id}`);
...
async function startConnection(profileId, onQR, onStatusChange, forceNew = false) {
  const sessionPath = getSessionPath(profileId);

  if (forceNew) {
    await clearSession(profileId);
  } else {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      console.log(`📩 Message event received for ${profileId}. From: ${msg.key.remoteJid}, fromMe: ${msg.key.fromMe}`);

      if (!msg.message || msg.key.fromMe) {
        console.log('Skipping: no message content or sent by us (fromMe=true)');
        return;
      }

      const customerPhone = msg.key.remoteJid.split('@')[0];
      const incomingText = msg.message.conversation || msg.message.extendedTextMessage?.text || '';
      console.log(`Incoming text: "${incomingText}" from ${customerPhone}`);
      if (!incomingText) {
        console.log('Skipping: empty text (probably media/other message type)');
        return;
      }
      ...
      if (blacklisted.rows.length > 0) {
        console.log('Skipping: number is blacklisted');
        return;
      }

      const profileResult = await db.query('SELECT * FROM profiles WHERE profile_id=$1', [profileId]);
      const profile = profileResult.rows[0];
      if (!profile || !profile.bot_active) {
        console.log(`Skipping: bot_active=${profile?.bot_active}`);
        return;
      }
...
module.exports = { startConnection, getConnection, logout, resumeAllConnections, clearSession };
